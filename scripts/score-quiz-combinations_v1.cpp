...
// Archetype dimension profiles from archetypeData.js (v1 - ORIGINAL)
struct Archetype {
    std::string id;
    std::string name;
...
// Quiz answer to archetype mapping (v1 - ORIGINAL)
std::vector<int> quizAnswerToArchetype = {
    ...
    0,  // Q8: A1 -> Orchestrator
    1,  // Q8: A2 -> Generalist
    2,  // Q8: A3 -> Researcher
    3,  // Q8: A4 -> Experimentalist
    4,  // Q9: A1 -> Director
    5,  // Q9: A2 -> Disruptor
    6,  // Q9: A3 -> Educator
    7,  // Q9: A4 -> Connector
...
int main() {
    ...
    std::cout << "🎯 Scoring " << TOTAL_COMBINATIONS << " quiz answer combinations (v1 - cosine)..." << std::endl;
    std::cout << "📁 Input: src/test/fixtures/quiz_answer_combinations.csv.gz" << std::endl;
    std::cout << "📁 Output: src/test/fixtures/quiz_results_v1.csv" << std::endl << std::endl;
    ...
    std::ofstream outfile("../src/test/fixtures/quiz_results_v1.csv");
    if (!outfile.is_open()) {
    ...
    // Write stats to JSON
    std::ofstream statsfile("../src/test/fixtures/quiz_results_v1_stats.json");
    statsfile << "{\n";
    ...
    std::cout << "\nDetailed tie breakdown in: ../src/test/fixtures/quiz_results_v1_stats.json" << std::endl;
